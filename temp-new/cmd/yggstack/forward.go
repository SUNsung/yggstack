...
const (
	proxyTCPCloseTimeout = 5 * time.Second
...
func startLocalUDP(ctx context.Context, node core.Interface, mappings []udpMappingObj, log yggcore.Logger, wg *sync.WaitGroup) {
	for _, m := range mappings {
		wg.Add(1)
...
func startRemoteUDP(ctx context.Context, node core.Interface, mappings []udpMappingObj, log yggcore.Logger, wg *sync.WaitGroup) {
	for _, m := range mappings {
		wg.Add(1)
		go func(m udpMappingObj) {
			defer wg.Done()
...
func runUDPLoop(ctx context.Context, listenConn net.PacketConn, dialFn func() (net.Conn, error), mtu uint64, log yggcore.Logger) {
	sessions := sync.Map{}
		...
		for {
			...
			case <-ticker.C:
				...
				sessions.Range(func(k, v any) bool {
					s := v.(*udpSessionObj)
					if now-s.lastActivity > udpSessionTimeout.Milliseconds() {
