...
graph LR
    subgraph nicObj
        ...
        RSTQueue[RST queue<br/>chan 100]
        WriteBufPool[sync.Pool<br/>65535 bytes]
    end
...
(`chan *PacketBuffer`, ёмкость 100). Это позволяет не блокировать gVisor при отправке RST.
...
4. Если снова неудача — пакет отбрасывается с debug-логированием

**sync.Pool для записи:** буферы для `writePacket` аллоцируются через `sync.Pool` (65535 байт),
что минимизирует нагрузку на GC при высокой частоте пакетов. Паника в `writePacket` перехватывается через `recover`
...
| `MaxConnections` | int    | `0`          | Максимум одновременных соединений. `0` — без ограничений. При достижении лимита новые соединения сбрасываются с логированием |
...
```mermaid
flowchart TD
    ACCEPT[Accept соединение] --> CHECK{Семафор свободен?}
    CHECK -->|Да| SLOT[Занять слот] --> SERVE[Обслужить через SOCKS5]
    SERVE --> CLOSE[Закрыть соединение] --> FREE[Освободить слот]
    CHECK -->|Нет| DROP[Закрыть соединение] --> LOG[Лог: limit reached]
    LOG --> ACCEPT
```

- Проверка неблокирующая — Serve-loop не зависает при полном семафоре
- Соединение закрывается сразу, клиент получает TCP RST
- Каждый сброс логируется через `Logger.Infof`
...
| `Close`           | Всегда `nil` (ошибки компонентов игнорируются при завершении)           |
